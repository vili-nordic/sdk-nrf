...
psa_algorithm_t psa_alg = PSA_ALG_SPAKE2P_HMAC(PSA_ALG_SHA_256);
int main(void)
{
	...
	psa_pake_cs_set_algorithm(&cipher_suite, psa_alg);
	...
	/* Client side pake setup. */
	{
		status = psa_pake_setup(&client_op, key, &cipher_suite);
		if (status != PSA_SUCCESS) {
			LOG_INF("psa_pake_setup failed. (Error: %d)", status);
			goto error;
		}

		status = psa_pake_set_role(&client_op, PSA_PAKE_ROLE_CLIENT);
		if (status != PSA_SUCCESS) {
			LOG_INF("psa_pake_set_role failed. (Error: %d)", status);
			goto error;
		}

		status = psa_pake_set_user(&client_op, "client", strlen("client"));
		if (status != PSA_SUCCESS) {
			LOG_INF("psa_pake_set_user failed. (Error: %d)", status);
			goto error;
		}

		status = psa_pake_set_peer(&client_op, "server", strlen("server"));
		if (status != PSA_SUCCESS) {
			LOG_INF("psa_pake_set_peer failed. (Error: %d) 123", status);
			goto error;
		}
	}

	psa_pake_operation_t server_op = PSA_PAKE_OPERATION_INIT;
	/* Server side pake setup. */
	{
		status = psa_pake_setup(&server_op, key_server, &cipher_suite);
		if (status != PSA_SUCCESS) {
			LOG_INF("psa_pake_setup failed. (Error: %d)", status);
			goto error;
		}

		status = psa_pake_set_role(&server_op, PSA_PAKE_ROLE_SERVER);
		if (status != PSA_SUCCESS) {
			LOG_INF("psa_pake_set_role failed. (Error: %d)", status);
			goto error;
		}

		status = psa_pake_set_peer(&server_op, "client", strlen("client"));
		if (status != PSA_SUCCESS) {
			LOG_INF("psa_pake_set_peer failed. (Error: %d)", status);
			goto error;
		}

		status = psa_pake_set_user(&server_op, "server", strlen("server"));
		if (status != PSA_SUCCESS) {
			LOG_INF("psa_pake_set_user failed. (Error: %d)", status);
			goto error;
		}
	...
	struct {
		...
		{&client_op, client_secret},
		{&server_op, server_secret}
	...
	for (size_t i = 0; i < 2; i++)
	{
		...
		status = psa_key_derivation_input_key(&kdf, PSA_KEY_DERIVATION_INPUT_SECRET,
						      key);
		if (status != PSA_SUCCESS) {
		...
		status = psa_key_derivation_output_bytes(&kdf, client_server[i].secret, sizeof client_secret);
		if (status != PSA_SUCCESS) {
